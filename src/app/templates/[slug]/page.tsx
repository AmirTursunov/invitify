...
export default async function TemplateDetailPage({ params }: Props) {
  ...
  Object.entries(fields as Record<string, any>).forEach(([key, f]) => {
    if (key.includes("Name") || key.includes("groom") || key.includes("bride"))
      sampleData[key] = f.placeholder || "Azizbek";
    ...
    if (key.includes("address"))
      sampleData[key] = "Toshkent shahri, Yakkasaroy tumani 15";
    if (key.includes("phone")) sampleData[key] = "+998 90 123 45 67";
    if (key.includes("message"))
      sampleData[key] = "Sizlarni ushbu quvonchli kunimizda kutib qolamiz!";
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-start">
          ...
          <div className="flex flex-col order-2 lg:order-1">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 mb-4 tracking-tight leading-tight">
              {template.name}
              {template.isPremium && (
                <Crown className="inline-block w-8 h-8 text-amber-500 ml-3 mb-2" />
              )}
            </h1>
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 border border-gray-100 mb-8 shadow-sm">
              <div className="flex items-end justify-between mb-6 pb-6 border-b border-gray-100">
                <div>
                  ...
                  <span className="text-3xl md:text-4xl font-black text-gray-900">
                    {template.price === 0
                      ? "Bepul"
                      : `${(template.price / 100).toLocaleString()} so'm`}
                  </span>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3 mb-8">
                {[
                  "Ajoyib zamonaviy dizayn",
                  "Mobil va Web moslashuvchan",
                  "QR kodli vizitka uslubi",
                  "Cheksiz marta ulashish",
                  "To'lovdan so'ng rasm ko'rinishida yuklash",
                ].map((item) => (
                  <div
                    key={item}
              ...
              <Link
                href={
                  session
                    ...
                    : `/auth/login?redirect=/dashboard/create?template=${templateId}`
                }
            ...
            <div className="bg-white/50 rounded-2xl p-6 border border-gray-100">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                <span className="bg-purple-100 text-purple-700 w-6 h-6 rounded flex items-center justify-center text-xs">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(fields).map(([key, field]) => (
                  <div
                    key={key}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${field.required ? "bg-white border-purple-200 text-purple-700" : "bg-transparent border-gray-200 text-gray-500"}`}
                  >
                    {field.label}{" "}
                    {field.required && (
