...
import DeleteInvitationButton from "@/components/ui/DeleteInvitationButton";
...
export default async function InvitationDetailPage({ params }: Props) {
  ...
  const statusInfo = INVITATION_STATUS_LABELS[inv.status] || {
    label: inv.status,
    color: "gray",
  };
  const orderStatusInfo = order ? ORDER_STATUS_LABELS[order.status] : null;
  const canOrder = inv.status === "DRAFT" && !order;
  const isPaid = ["ACTIVE", "PAID"].includes(inv.status) || order?.status === "PAID";
        ...
        <div className="flex items-center gap-2">
          {!isPaid && (
            <Link
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 rounded-lg text-xs font-bold transition-all"
            >
              <Edit2 className="w-3.5 h-3.5" /> O'zgartirish
            </Link>
          )}
      ...
      <div className="grid gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`text-sm px-3 py-1 rounded-full font-medium ${statusInfo.color === "green" ? "bg-green-100 text-green-700" : statusInfo.color === "yellow" ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-600"}`}
            >
              {statusInfo.label}
            </span>
          </div>
          <div className="flex items-center gap-2">
            {["Qoralama", "To'lov", "Faol"].map((s, i) => {
              const isDone =
                (i === 0 &&
                  ["PENDING_PAYMENT", "PAID", "ACTIVE"].includes(inv.status)) ||
                (i === 1 && ["PAID", "ACTIVE"].includes(inv.status)) ||
                (i === 2 && inv.status === "ACTIVE");
              return (
                <div key={s} className="flex items-center gap-2 flex-1">
                  <div
                    className={`w-full h-2 rounded-full ${isDone ? "bg-purple-500" : "bg-gray-200"}`}
                  />
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {inv.publicUrl && (
              <CopyButton text={inv.publicUrl} label="Havolani nusxalash" />
            )}
          </div>
          {inv.publicUrl && (
            <div className="mt-3 p-3 bg-gray-50 rounded-xl text-xs text-gray-500 break-all font-mono">
              {inv.publicUrl}
            </div>
          )}
          ...
          <DownloadImageButton
            ...
            data={inv.data}
            isPaid={
              ["ACTIVE", "PAID"].includes(inv.status) ||
              order?.status === "PAID"
            }
            title={inv.title}
          />
        ...
        {order ? (
            ...
            <div className="space-y-3">
              ...
              {orderStatusInfo && (
                <div className="flex justify-between text-sm">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-medium ${orderStatusInfo.color === "green" ? "bg-green-100 text-green-700" : orderStatusInfo.color === "yellow" ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-600"}`}
                  >
              ...
              {order.status === "PENDING" && (
                <div className="mt-4 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                  ...
                  <p className="text-xs text-yellow-600 mt-2 font-mono bg-yellow-100 p-2 rounded">
                    Izoh: Buyurtma ID: {order._id.toString()}
                  </p>
                </div>
              )}
              {order.adminNote && (
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Taklifnomangizni faollashtirish uchun to'lov qiling
            </p>
            <div className="flex items-center justify-between p-4 bg-purple-50 rounded-xl mb-4">
              ...
              <span className="text-xl font-bold text-purple-700">
                {template?.price === 0
                  ? "Bepul"
                  : `${(template?.price || 0).toLocaleString()} so'm`}
              </span>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(inv.data as Record<string, string>)
              .filter(([, v]) => v)
              .map(([key, value]) => (
                <div key={key} className="text-sm">
                  <div className="text-gray-400 text-xs capitalize mb-0.5">
